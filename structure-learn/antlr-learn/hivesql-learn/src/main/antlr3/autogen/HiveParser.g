...
deleteTableStatement
@init { pushMsg("delete table statement", state); }
@after { popMsg(state); }
    : KW_DELETE KW_TABLE ifExists? tableName KW_PURGE? replicationClause?
    -> ^(TOK_DELETETABLE tableName ifExists? KW_PURGE? replicationClause?)
    ;
